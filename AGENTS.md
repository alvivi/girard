...
| `src/girard/internal/type_.gleam` | The inference-side `Type` and `Scheme`: the public vocabulary plus the narrowed-variant slot on `Named`, converted to the public types when a result or an error is published |
...
`Type` from `girard/internal/type_`; the public `Type` is produced from it
